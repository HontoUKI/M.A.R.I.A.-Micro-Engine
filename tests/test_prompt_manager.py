...
def test_last_message_is_user_tail_carrying_steering_and_message():
    ...
    assert "warm, curious" in last["content"]
...
def test_section_labels_can_be_disabled():
    ...
    unlabeled = PromptManager(estimate_tokens=_words, section_label=False)
    tail = unlabeled.build_messages(inputs)[-1]["content"]
    assert "[State]" not in tail
    assert "warm" in tail and "hi" in tail
...
def test_memory_recall_dropped_before_dialogue_when_budget_tight():
    ...
    messages = _manager(max_tokens=12).build_messages(inputs)
